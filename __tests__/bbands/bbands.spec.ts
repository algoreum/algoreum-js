...
import { BollingerBands } from '../../src/ta';
...
describe('Bollinger Bands', () => {
  it('Excel Validate', () => {
    ...
    ohlc.forEach((tick, idx) => {
      ...
      if (!excel && !calculated) {
        ...
        expect(Math.abs(calculated.middle - excel.middle)).toBeLessThan(
          EPSILON
        );
  ...
  it('Cross SDK validate', () => {
    ...
    ohlc.forEach((tick) => {
      const local = bb1.next(tick.c);
      const cross = bb2.nextValue(tick.c);

      expect(Math.abs((local?.lower || 0) - (cross?.lower || 0))).toBeLessThan(
        0.000001
      );
      expect(
        Math.abs((local?.middle || 0) - (cross?.middle || 0))
      ).toBeLessThan(0.000001);
      expect(Math.abs((local?.upper || 0) - (cross?.upper || 0))).toBeLessThan(
        0.000001
      );
    });
  });
