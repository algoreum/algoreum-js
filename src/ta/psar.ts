...
export class PSAR {
  ...
  next(high: number, low: number, close: number) {
    ...
    if (this.isBullTrend) {
      this.psar =
        this.result + this.accelerationFactor * (this.highest - this.result);
    } else {
      this.psar =
        this.result + this.accelerationFactor * (this.lowest - this.result);
    }
      ...
      if (low < this.psar) {
        ...
        if (high > this.highest) {
          ...
          this.accelerationFactor =
            sumOfAcceleration > this.max ? this.max : sumOfAcceleration;
        }
